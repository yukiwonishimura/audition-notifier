...
# 重要: この段階では点数付け・ランク判定・「通知に値するか」の選別をしないこと
見つけた応募可能な案件は、規模が小さくても無名の制作でも**すべて**配列に含める。
評価と選別は後段の別工程が行う。ここで除外してよいのは次の3つだけ:
  (a) 募集が明確に終了している / 締切を過ぎている
  (b) 年齢・性別が明らかに不一致（30代男性が応募できる役が一切ない）
  (c) 高額登録料など明らかに悪質な募集（その場合は配列に含めた上で suspicious=true）
...
応募可能な案件が本当に1件も無いときだけ [] を出力する。JSONコードブロック以外に配列を書かない。`;
